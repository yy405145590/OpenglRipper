...
template<typename T, size_t buffer_size = 16, typename index_t = size_t, bool bUseLock = true>
class Ringbuffer
{
public:
	Ringbuffer() 
	...
	bool skip(int size)
	{
		int newReadSize = readAvailable() - size;
		assert(newReadSize >= 0 && newReadSize <= buffer_size);
		if (newReadSize >= 0 && newReadSize <= buffer_size)
		{
			tail += size;
			return true;
		}
...
template<typename T, size_t buffer_size, typename index_t, bool bUseLock = true>
size_t Ringbuffer<T, buffer_size, index_t, bUseLock>::writeBuff(T* buff, size_t count, size_t count_to_callback,
...
template<typename T, size_t buffer_size, typename index_t, bool bUseLock = true>
size_t Ringbuffer<T, buffer_size, index_t, bUseLock>::readBuff(T* buff, size_t count, size_t count_to_callback,
	void(*execute_data_callback)())
{
	size_t read = 0;
	index_t available = 0;
