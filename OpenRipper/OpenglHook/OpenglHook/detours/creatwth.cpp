...
typedef struct _DETOUR_EXE_RESTORE64
{
    ULONG               cb;
    ...
    ULONG               clrFlags;
} DETOUR_EXE_RESTORE64, *PDETOUR_EXE_RESTORE64;


typedef struct _DETOUR_EXE_RESTORE32
{
    ULONG               cb;
    ...
    ULONG               clrFlags;
} DETOUR_EXE_RESTORE32, *PDETOUR_EXE_RESTORE32;



BOOL WINAPI DetourCopyPayloadToProcess64(HANDLE hProcess,
                                       REFGUID rguid,
                                       PVOID pData,
                                       DWORD cbData)
{
...
static WORD detour_sum_final64(WORD wSum, PIMAGE_NT_HEADERS64 pinh)
{
    DETOUR_TRACE((".... : %08x (value: %08x)\n", wSum, pinh->OptionalHeader.CheckSum));
...
static BOOL IsExe64(HANDLE hProcess, PBYTE pbModule)
{
    IMAGE_DOS_HEADER idh;
    ZeroMemory(&idh, sizeof(idh));
...
PVOID FindExe64(HANDLE hProcess)
{
    MEMORY_BASIC_INFORMATION mbi;
    ZeroMemory(&mbi, sizeof(mbi));
...
static WORD ComputeChkSum64(HANDLE hProcess, PBYTE pbModule, PIMAGE_NT_HEADERS64 pinh)
{
    // See LdrVerifyMappedImageMatchesChecksum.
    ...
    return detour_sum_final64(wSum, pinh);
}
...
BOOL WINAPI UpdateImports64(HANDLE hProcess, LPCSTR *plpDlls, DWORD nDlls)
{
    BOOL fSucceeded = FALSE;
    BYTE * pbNew = NULL;
    DETOUR_EXE_RESTORE64 der;
    DWORD i;

    ZeroMemory(&der, sizeof(der));
    der.cb = sizeof(der);

    PBYTE pbModule = (PBYTE)FindExe64(hProcess);
    ...
    CopyMemory(&der.inh, &inh, sizeof(inh));
    der.pinh = (PIMAGE_NT_HEADERS64)(pbModule + idh.e_lfanew);
    ...
    // Find the current checksum.
    WORD wBefore = ComputeChkSum64(hProcess, pbModule, &inh);
    DETOUR_TRACE(("ChkSum: %04x + %08x => %08x\n", wBefore, dwFileSize, wBefore + dwFileSize));
    ...
    for (n = 0; n < nDlls; n++) {
        ...
        DWORD nOffset = obTab + (sizeof(DWORD_PTR) * (4 * n));
        piid[n].OriginalFirstThunk = obBase + nOffset;
        pt = ((ULONGLONG*)(pbNew + nOffset));
        pt[0] = IMAGE_ORDINAL_FLAG64 + 1;
        pt[1] = 0;
    ...
    /////////////////////// Update the NT HEADER64 for the import new directory.
    /////////////////////////////// Update the DOS HEADER64 to fix the checksum.
    //
    if (!VirtualProtectEx(hProcess, pbModule, inh.OptionalHeader.SizeOfHeaders,
                          PAGE_EXECUTE_READWRITE, &dwProtect)) {
        DETOUR_TRACE(("VirtualProtextEx(inh) write failed: %d\n", GetLastError()));
    ...
    WORD wDuring = ComputeChkSum64(hProcess, pbModule, &inh);
    DETOUR_TRACE(("ChkSum: %04x + %08x => %08x\n", wDuring, dwFileSize, wDuring + dwFileSize));
    ...
    WORD wAfter = ComputeChkSum64(hProcess, pbModule, &inh);
    DETOUR_TRACE(("ChkSum: %04x + %08x => %08x\n", wAfter, dwFileSize, wAfter + dwFileSize));
    DETOUR_TRACE(("Before: %08x, After: %08x\n", wBefore + dwFileSize, wAfter + dwFileSize));
    ...
    if (!DetourCopyPayloadToProcess64(hProcess, DETOUR_EXE_RESTORE_GUID, &der, sizeof(der))) {
        DETOUR_TRACE(("DetourCopyPayloadToProcess failed: %d\n", GetLastError()));
        goto finish;
    }
...
BOOL WINAPI DetourCopyPayloadToProcess32(HANDLE hProcess,
                                       REFGUID rguid,
                                       PVOID pData,
                                       DWORD cbData)
{
...
static BOOL IsExe32(HANDLE hProcess, PBYTE pbModule)
{
    IMAGE_DOS_HEADER idh;
    ZeroMemory(&idh, sizeof(idh));
...
PVOID FindExe32(HANDLE hProcess)
{
    MEMORY_BASIC_INFORMATION mbi;
    ZeroMemory(&mbi, sizeof(mbi));
...
static WORD detour_sum_final32(WORD wSum, PIMAGE_NT_HEADERS32 pinh)
{
    DETOUR_TRACE((".... : %08x (value: %08x)\n", wSum, pinh->OptionalHeader.CheckSum));
...
static WORD ComputeChkSum32(HANDLE hProcess, PBYTE pbModule, PIMAGE_NT_HEADERS32 pinh)
{
    // See LdrVerifyMappedImageMatchesChecksum.
    ...
    return detour_sum_final32(wSum, pinh);
}
...
BOOL WINAPI UpdateImports32(HANDLE hProcess, LPCSTR *plpDlls, DWORD nDlls)
{
    BOOL fSucceeded = FALSE;
    BYTE * pbNew = NULL;
    DETOUR_EXE_RESTORE32 der;
    DWORD i;

    ZeroMemory(&der, sizeof(der));
    der.cb = sizeof(der);

    PBYTE pbModule = (PBYTE)FindExe32(hProcess);
    ...
    CopyMemory(&der.inh, &inh, sizeof(inh));
    der.pinh = (PIMAGE_NT_HEADERS32)(pbModule + idh.e_lfanew);
    ...
    // Find the current checksum.
    WORD wBefore = ComputeChkSum32(hProcess, pbModule, &inh);
    DETOUR_TRACE(("ChkSum: %04x + %08x => %08x\n", wBefore, dwFileSize, wBefore + dwFileSize));
    ...
    /////////////////////// Update the NT HEADER32 for the import new directory.
    /////////////////////////////// Update the DOS HEADER32 to fix the checksum.
    //
    if (!VirtualProtectEx(hProcess, pbModule, inh.OptionalHeader.SizeOfHeaders,
                          PAGE_EXECUTE_READWRITE, &dwProtect)) {
        DETOUR_TRACE(("VirtualProtextEx(inh) write failed: %d\n", GetLastError()));
    ...
    WORD wDuring = ComputeChkSum32(hProcess, pbModule, &inh);
    DETOUR_TRACE(("ChkSum: %04x + %08x => %08x\n", wDuring, dwFileSize, wDuring + dwFileSize));
    ...
    WORD wAfter = ComputeChkSum32(hProcess, pbModule, &inh);
    DETOUR_TRACE(("ChkSum: %04x + %08x => %08x\n", wAfter, dwFileSize, wAfter + dwFileSize));
    DETOUR_TRACE(("Before: %08x, After: %08x\n", wBefore + dwFileSize, wAfter + dwFileSize));
    ...
    if (!DetourCopyPayloadToProcess32(hProcess, DETOUR_EXE_RESTORE_GUID, &der, sizeof(der))) {
        DETOUR_TRACE(("DetourCopyPayloadToProcess failed: %d\n", GetLastError()));
        goto finish;
    }
